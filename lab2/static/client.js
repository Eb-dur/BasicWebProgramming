let SIGNUP_PASSWORDS_MATCH = false;
let LOGGED_IN_STATE = 1;

disppayView = function () {
...
function loadWelcomeView() {
    ...
    // On input event to see if passwords match in sign up.
    document.getElementById('rpsw').oninput = function () {
    ...
    document.getElementById("signupform").onsubmit = function () {
        ...
        if (!SIGNUP_PASSWORDS_MATCH) { return false; }
...
function loadBrowseWall(email) {
    ...
    if (messages.success) {
        ...
        if (messages.data.length > 0) {
            ...
            let profileData = serverstub.getUserDataByEmail(userToken, email).data;
